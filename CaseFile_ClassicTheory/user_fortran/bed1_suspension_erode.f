...
!           CE N'EST PAS FAIT ACTUELLMENT DANS SISYPHE POUR LA SUSPENSION
            DO IPOIN=1,NPOIN
              !PRINT *,'ratio mud sand',RATIO_MUD_SAND(ILAYER,IPOIN) 
...
!         PRINT *,'NMUD',NMUD,'NSAND',NSAND,'NSAND_VIRTUAL',NSAND_VIRTUAL
!         PRINT *,'XWC',XWC(1),'UNORM',UNORM%R(IPOIN),'HN',HN%R(IPOIN)
!         PRINT *,'TAUP',TAUP%R(IPOIN),NPOIN,IPOIN
          IF(NMUD.GT.0)THEN
            DO ISAND = 1,NSAND_VIRTUAL
               !DELEEUW ENTRAINMENT HERE
               FLUER_PUR_MUD(ISAND,IPOIN) = 7.04D-4*XMVS0(ISAND)*
     &           (SQRT(TAUP%R(IPOIN)/XMVE)/XWC(ISAND))**1.71D0
...
!
          DO ISAND = 1,NSAND_VIRTUAL
!     !!!!!REMOVED HARD CODE CRITICAL SHEAR FOR SAND HERE!!!!!!!
!             PRINT *,'RATIO MUD ALL', RATIO_MUD_SAND(ILAYER,IPOIN)
!             PRINT *,'DENSITY S',XMVS0(ISAND),NSAND_VIRTUAL
c$$$            IF(TAUP%R(IPOIN).GT.TOCE_MIX(ISAND,ILAYER,IPOIN))THEN
c$$$              IF(RATIO_MUD_SAND(ILAYER,IPOIN).LE.0.3D0)THEN
!               MUD RATIO < 30%, (PURE SAND FLUX)
                FLUER_MIX(ISAND,IPOIN)= FLUER_PUR_SAND(ISAND,IPOIN)
!               MUD RATIO > 50%, (PURE MUD FLUX)
c$$$              ELSEIF(RATIO_MUD_SAND(ILAYER,IPOIN).GE.0.5D0)THEN
c$$$                FLUER_MIX(ISAND,IPOIN) = FLUER_PUR_MUD(ISAND,IPOIN)
c$$$!               MUD RATIO >30% AND <50%, (INTERPOLATION)
c$$$              ELSE
c$$$                FLUER_MIX(ISAND,IPOIN) = (RATIO_MUD_SAND(ILAYER,IPOIN)
c$$$     &               - 0.3D0)/(0.5D0-0.3D0)
c$$$     &               *(FLUER_PUR_MUD(ISAND,IPOIN)
c$$$     &                 -FLUER_PUR_SAND(ISAND,IPOIN))
c$$$     &               + FLUER_PUR_SAND(ISAND,IPOIN)
c$$$              ENDIF
c$$$            ELSE
c$$$              FLUER_MIX(ISAND,IPOIN) = 0.D0
c$$$            ENDIF
!
...
!         EXIT LAYER LOOP
          DO ISAND = 1,NSAND_VIRTUAL
!             PRINT *, 'RATIO_SAND',RATIO_SAND(ISAND,ILAYER,IPOIN)
...
! PRINT*,'QERSAND',QER_SAND(ISAND),'CHK POS',CHECK_POSITIF_SAND
